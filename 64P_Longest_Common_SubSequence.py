...
class OmniCreate:
    ...
    def longestSubSequence_recursion(self, ekahs, duva, m, n):
        ...
        if ekahs[m] == duva[n]:
            ...
            return max(
                self.longestSubSequence_recursion(ekahs, duva, m, n - 1),
                self.longestSubSequence_recursion(ekahs, duva, m - 1, n),
            )
...
print(
    ...
    create.longestSubSequence_iteration(animated, bnimated),
)
print(
    "Using Recursion Programming=",
    create.longestSubSequence_recursion(
        animated, bnimated, len(animated) - 1, len(bnimated) - 1
    ),
)
